...
}, "Por favor indica una fecha v&aacute;lida");


jQuery.validator.addMethod("dateTimeES", function(value, element) {
	var check = false,
		re = /^\d{1,2}\/\d{1,2}\/\d{4} [0-2][0-9]:[0-5][0-9]$/,
		adata, gg, mm, aaaa, xdata, fecha, hora, hdata, ihora, imin;
	
	if ( re.test(value)) {
		fecha = value.substring(0,10);
		hora = value.substring(11);
		hdata = hora.split(":");
		ihora = parseInt(hdata[0],10);
		imin = parseInt(hdata[1],10);
...
jQuery.validator.addMethod( "nifES", function( value ) {
	...
	// Basic format test
	if (( !value.match("((^[A-Z]{1}[0-9]{7}[A-Z0-9]{1}$|^[T]{1}[A-Z0-9]{8}$)|^[0-9]{8}[A-Z]{1}$)") ) && (value !='')){
...
}, "Por favor indica un NIF v&aacute;lido" );
...
}, "Por favor indica una hora v&aacute;lida, entre 00:00 y 23:59");
